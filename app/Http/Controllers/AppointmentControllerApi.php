...
class AppointmentControllerApi extends AppBaseController
{
     ...
     * @return Application|Factory|View
     */
    //inorder to make an appointment you need to have a token as in you have to be logged in. 
    ...
    //the app can also send data and it can be accesed through the request method. 
        $input = $request->all();
        
//this here write the appointment into the database 
    ...
    //TODO generate the doctor id , date of the appointment from time , to time , description , service id from service selection , 

    public function storeApi(CreateAppointmentRequest $request ){
        $input = $request->all();
        $user= Doctor::where('user_id',$input['doctor_id'])->first();
        $doctor_id=  $user->id;
        //this here write the appointment into the database 
...
//actual creating of appointment 
     ...
     */
     //TODO  THIS CAN BE USED TO DISPLAY AN INDIVIDUAL APPOINTMEMNT INFORMATION 
     ...
     * @return Application|Factory|View
     */
    public function patientAppointmentCalendar($id)
    {
        // if ($request->ajax()) {
            // $input = $request->all();
            $data = $this->appointmentRepository->getPatientAppointmentsCalendar($id);
